...
class BrainyUIBox extends FlxSpriteGroup
{
	...
	public var tabs(default, null):Array<BrainyUITab> = [];
	
	public var selectedTab(default, set):BrainyUITab = null;
	...
	public function new(x:Float, y:Float, width:Int, height:Int, tabs:Array<String> = null)
	{
		...
		if(tabs != null)
		{
			for (tab in tabs)
			{
				var createdTab:BrainyUITab = new BrainyUITab(tab);
	...
	override function update(elapsed:Float)
	{
		...
		else
		{
			...
			if(wasDragging && broadcastBoxEvents) BrainyUIEventHandler.event(DROP_EVENT, this);
		}
			...
			for (tab in tabs)
			{
				if(FlxG.mouse.overlaps(tab, camera))
				{
					...
					if(!_draggingBox && canMove && _pressedBox && FlxG.mouse.pressed && (Math.abs(FlxG.mouse.deltaScreenX) > 1 || Math.abs(FlxG.mouse.deltaScreenY) > 1))
					{
						...
						_draggingBox = true;
						if(broadcastBoxEvents) BrainyUIEventHandler.event(DRAG_EVENT, this);
					}
					...
					if(FlxG.mouse.justPressed)
					{
						...
						selectedTab = tab;
						_lastClick = 0;
						if(broadcastBoxEvents) BrainyUIEventHandler.event(CLICK_EVENT, this);
					}
		...
		if(_ignoreTabUpdate)
		{
			if(broadcastBoxEvents)
				BrainyUIEventHandler.event(MINIMIZE_EVENT, this);
		}
	...
	{
		var createdTab:BrainyUITab = new BrainyUITab(name);
	...
	private function set_selectedTab(v:BrainyUITab)
	{
		if(v != null)
		{
