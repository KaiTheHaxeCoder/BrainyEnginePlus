...
class BrainyUIDropDownMenu extends BrainyUIInputText
{
	...
	var _items:Array<BrainyUIDropDownItem> = [];
	public var curScroll:Int = 0;
	override function update(elapsed:Float)
	{
		var lastFocus = BrainyUIInputText.focusOn;
		...
		if(FlxG.mouse.justPressed)
		{
			if(FlxG.mouse.overlaps(button, camera))
			{
				...
				if(lastFocus != this)
					BrainyUIInputText.focusOn = this;
				else if(BrainyUIInputText.focusOn == this)
		...
		if(lastFocus != BrainyUIInputText.focusOn)
		{
			showDropDown(BrainyUIInputText.focusOn == this);
		}
		else if(BrainyUIInputText.focusOn == this)
		{
	...
	function clickedOn(num:Int, label:String)
	{
		...
		if(broadcastDropDownEvent) BrainyUIEventHandler.event(CLICK_EVENT, this);
	}
	...
	function addOption(option:String)
	{
		...
		var item:BrainyUIDropDownItem = cast recycle(BrainyUIDropDownItem, () -> new BrainyUIDropDownItem(1, 1, this._itemWidth), true);
...
class BrainyUIDropDownItem extends FlxSpriteGroup
{
