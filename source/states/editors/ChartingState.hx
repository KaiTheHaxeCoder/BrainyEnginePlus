...
class ChartingState extends MusicBeatState implements BrainyUIEventHandler.BrainyUIEvent
{
	...
	[
		...
		["Show Lyric", "Value 1: Lyric to show"],
		["Hide Lyric", "Hides the lyric...thats it."]
	];
	...
	override function create()
	{
		...
		infoBox = new BrainyUIBox(infoBoxPosition.x, infoBoxPosition.y, 220, 220, ['Information']);
		...
		mainBox = new BrainyUIBox(mainBoxPosition.x, mainBoxPosition.y, 300, 280, ['Charting', 'Data', 'Events', 'Note', 'Section', 'Song']);
		...
		upperBox = new BrainyUIBox(40, 40, 330, 300, ['File', 'Edit', 'View']);
	...
	var lastFocus:BrainyUIInputText;
	...
	override function update(elapsed:Float)
	{
		...
		{
			lastFocus = BrainyUIInputText.focusOn;
			return;
		}
		...
		{
			if(BrainyUIInputText.focusOn == null) //If not typing anything
			{
		...
		if(BrainyUIInputText.focusOn == null && lastFocus == null)
		{
	...
	var playbackSlider:BrainyUISlider;

	var mouseSnapCheckBox:BrainyUICheckBox;
	var ignoreProgressCheckBox:BrainyUICheckBox;
	var hitsoundPlayerStepper:BrainyUINumericStepper;
	var hitsoundOpponentStepper:BrainyUINumericStepper;
	var metronomeStepper:BrainyUINumericStepper;

	var instVolumeStepper:BrainyUINumericStepper;
	var instMuteCheckBox:BrainyUICheckBox;
	var playerVolumeStepper:BrainyUINumericStepper;
	var playerMuteCheckBox:BrainyUICheckBox;
	var opponentVolumeStepper:BrainyUINumericStepper;
	var opponentMuteCheckBox:BrainyUICheckBox;
	function addChartingTab()
	{
		...
		objY += 25;
		playbackSlider = new BrainyUISlider(50, objY, function(v:Float) setPitch(playbackRate = v), 1, 0.1, 5.0, 200);
		playbackSlider.label = 'Playback Rate';
		
		objY += 60;
		mouseSnapCheckBox = new BrainyUICheckBox(objX, objY, 'Mouse Scroll Snap', 100, function() chartEditorSave.data.mouseScrollSnap = mouseSnapCheckBox.checked);
		mouseSnapCheckBox.checked = chartEditorSave.data.mouseScrollSnap;

		ignoreProgressCheckBox = new BrainyUICheckBox(objX + 150, objY, 'Ignore Progress Warnings', 100, function() chartEditorSave.data.ignoreProgressWarns = ignoreProgressCheckBox.checked);
		ignoreProgressCheckBox.checked = chartEditorSave.data.ignoreProgressWarns;

		objY += 50;
		hitsoundPlayerStepper = new BrainyUINumericStepper(objX, objY, 0.2, 0, 0, 1, 1);
		hitsoundOpponentStepper = new BrainyUINumericStepper(objX + 100, objY, 0.2, 0, 0, 1, 1);
		metronomeStepper = new BrainyUINumericStepper(objX + 200, objY, 0.2, 0, 0, 1, 1);

		objY += 50;
		instVolumeStepper = new BrainyUINumericStepper(objX, objY, 0.1, 0.6, 0, 1, 1);
		instVolumeStepper.onValueChange = updateAudioVolume;
		playerVolumeStepper = new BrainyUINumericStepper(objX + 100, objY, 0.1, 1, 0, 1, 1);
		playerVolumeStepper.onValueChange = updateAudioVolume;
		opponentVolumeStepper = new BrainyUINumericStepper(objX + 200, objY, 0.1, 1, 0, 1, 1);
		opponentVolumeStepper.onValueChange = updateAudioVolume;

		objY += 25;
		instMuteCheckBox = new BrainyUICheckBox(objX, objY, 'Mute', 60, updateAudioVolume);
		playerMuteCheckBox = new BrainyUICheckBox(objX + 100, objY, 'Mute', 60, updateAudioVolume);
		opponentMuteCheckBox = new BrainyUICheckBox(objX + 200, objY, 'Mute', 60, updateAudioVolume);
	...
	var gameOverCharDropDown:BrainyUIDropDownMenu;
	var gameOverSndInputText:BrainyUIInputText;
	var gameOverLoopInputText:BrainyUIInputText;
	var gameOverRetryInputText:BrainyUIInputText;
	var noRGBCheckBox:BrainyUICheckBox;
	var noteTextureInputText:BrainyUIInputText;
	var noteSplashesInputText:BrainyUIInputText;
	function addDataTab()
	{
		var tab_group = mainBox.getTab('Data').menu;
		var objX = 10;
		var objY = 25;
		gameOverCharDropDown = new BrainyUIDropDownMenu(objX, objY, [''], function(id:Int, character:String)
		{
		...
		objY += 40;
		gameOverSndInputText = new BrainyUIInputText(objX, objY, 120, '', 8);
		...
		objY += 40;
		gameOverLoopInputText = new BrainyUIInputText(objX, objY, 120, '', 8);
		...
		objY += 40;
		gameOverRetryInputText = new BrainyUIInputText(objX, objY, 120, '', 8);
		...
		objY += 35;
		noRGBCheckBox = new BrainyUICheckBox(objX, objY, 'Disable Note RGB', 100, updateNotesRGB);
		
		objY += 40;
		noteTextureInputText = new BrainyUIInputText(objX, objY, 120, '');
		...
		noteSplashesInputText = new BrainyUIInputText(objX + 140, objY, 120, '');
	...
	var eventDropDown:BrainyUIDropDownMenu;
	var value1InputText:BrainyUIInputText;
	var value2InputText:BrainyUIInputText;
	...
	function addEventsTab()
	{
		...
		eventDropDown = new BrainyUIDropDownMenu(objX, objY, [], function(id:Int, character:String)
		{
		...
		var objX2 = 140;
		var removeButton:BrainyUIButton = new BrainyUIButton(objX2, objY, '-', function()
		{
		...
		var addButton:BrainyUIButton = new BrainyUIButton(objX2 + 30, objY, '+', function()
		{
			genericEventButton(function(event:EventMetaNote)
			{
		...
		var leftButton:BrainyUIButton = new BrainyUIButton(objX2 + 80, objY, '<', function()
		{
			genericEventButton(function(event:EventMetaNote) curEventSelected = FlxMath.wrap(curEventSelected - 1, 0, event.events.length - 1));
		}, 20);
		var rightButton:BrainyUIButton = new BrainyUIButton(objX2 + 110, objY, '>', function()
		{
			genericEventButton(function(event:EventMetaNote) curEventSelected = FlxMath.wrap(curEventSelected + 1, 0, event.events.length - 1));
		}, 20);
		...
		objY += 70;
		value1InputText = new BrainyUIInputText(objX, objY, 120, '', 8);
		value1InputText.onChange = function(old:String, cur:String) changeEventsValue(cur, 1);
		value2InputText = new BrainyUIInputText(objX + 150, objY, 120, '', 8);
	...
	var susLengthStepper:BrainyUINumericStepper;
	var strumTimeStepper:BrainyUINumericStepper;
	var noteTypeDropDown:BrainyUIDropDownMenu;
	...
	function addNoteTab()
	{
		...
		susLengthStepper = new BrainyUINumericStepper(objX, objY, Conductor.stepCrochet / 2, 0, 0, Conductor.stepCrochet * 128, 1, 80);
		...
		objY += 40;
		strumTimeStepper = new BrainyUINumericStepper(objX, objY, Conductor.stepCrochet, 0, -5000, Math.POSITIVE_INFINITY, 3, 120);
		...
		objY += 40;
		noteTypeDropDown = new BrainyUIDropDownMenu(objX, objY, [], function(id:Int, changeToType:String)
		{
	...
	var mustHitCheckBox:BrainyUICheckBox;
	var gfSectionCheckBox:BrainyUICheckBox;
	var altAnimSectionCheckBox:BrainyUICheckBox;

	var changeBpmCheckBox:BrainyUICheckBox;
	var changeBpmStepper:BrainyUINumericStepper;
	var beatsPerSecStepper:BrainyUINumericStepper;

	function addSectionTab()
	{
		var affectNotes:BrainyUICheckBox = null;
		var affectEvents:BrainyUICheckBox = null;
		var copyLastSecStepper:BrainyUINumericStepper = null;
		...
		mustHitCheckBox = new BrainyUICheckBox(objX, objY, 'Must Hit Sec.', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.mustHitSection = mustHitCheckBox.checked;
			updateHeads(true);
		});
		gfSectionCheckBox = new BrainyUICheckBox(objX + 100, objY, 'GF Section', 70, function()
		{
			var sec = getCurChartSection();
			if(sec != null) sec.gfSection = gfSectionCheckBox.checked;
			updateHeads(true);
		});
		altAnimSectionCheckBox = new BrainyUICheckBox(objX + 200, objY, 'Alt Anim', 70, function()
		{
			var sec = getCurChartSection();
		...
		objY += 40;
		changeBpmCheckBox = new BrainyUICheckBox(objX, objY, 'Change BPM', 80, function()
		{
			var sec = getCurChartSection();
			if(sec != null)
			{
		...
		objY += 25;
		changeBpmStepper = new BrainyUINumericStepper(objX, objY, 1, 0, 1, 400, 3);
		...
		beatsPerSecStepper = new BrainyUINumericStepper(objX + 150, objY, 1, 4, 1, 16, 2);
		...
		objY += 40;
		var copyButton:BrainyUIButton = new BrainyUIButton(objX, objY, 'Copy Section', copyNotesOnSection.bind());
		var pasteButton:BrainyUIButton = new BrainyUIButton(objX + 100, objY, 'Paste Section', function()
		{
			pasteCopiedNotesToSection(affectNotes.checked, affectEvents.checked);
		});
		var clearButton:BrainyUIButton = new BrainyUIButton(objX + 200, objY, 'Clear', function()
		{
			for (note in curRenderedNotes)
			{
				if(note == null) continue;
		...
		objY += 25;
		affectNotes = new BrainyUICheckBox(objX, objY, 'Notes', 60);
		affectNotes.checked = true;
		affectEvents = new BrainyUICheckBox(objX + 100, objY, 'Events', 60);

		objY += 32;
		var copyLastSecButton:BrainyUIButton = new BrainyUIButton(objX, objY, 'Copy Last Section', function()
		{
		...
		copyLastSecStepper = new BrainyUINumericStepper(objX + 110, objY + 2, 1, 1, -999, 999, 0);
		
		objY += 40;
		var swapSectionButton:BrainyUIButton = new BrainyUIButton(objX, objY, 'Swap Section', function()
		{
		...
		var duetSectionButton:BrainyUIButton = new BrainyUIButton(objX + 100, objY, 'Duet Section', function()
		{
			var side:Int = -1;
		...
		var mirrorNotesButton:BrainyUIButton = new BrainyUIButton(objX + 200, objY, 'Mirror Notes', function()
		{
			var maxData:Int = GRID_COLUMNS_PER_PLAYER * GRID_PLAYERS;
			for (note in curRenderedNotes)
			{
	...
	var songNameInputText:BrainyUIInputText;
	var allowVocalsCheckBox:BrainyUICheckBox;

	var bpmStepper:BrainyUINumericStepper;
	var scrollSpeedStepper:BrainyUINumericStepper;
	var audioOffsetStepper:BrainyUINumericStepper;

	var stageDropDown:BrainyUIDropDownMenu;
	var playerDropDown:BrainyUIDropDownMenu;
	var opponentDropDown:BrainyUIDropDownMenu;
	var girlfriendDropDown:BrainyUIDropDownMenu;
	...
	function addSongTab()
	{
		...
		songNameInputText = new BrainyUIInputText(objX, objY, 100, 'None', 8);
		songNameInputText.onChange = function(old:String, cur:String) PlayState.SONG.song = cur;

		allowVocalsCheckBox = new BrainyUICheckBox(objX, objY + 20, 'Allow Vocals', 80, function()
		{
			PlayState.SONG.needsVoices = allowVocalsCheckBox.checked;
			loadMusic();
		});
		var reloadAudioButton:BrainyUIButton = new BrainyUIButton(objX + 120, objY, 'Reload Audio', function() loadMusic(true), 80);

		#if mac
		var reloadJsonButton:BrainyUIButton = new BrainyUIButton(objX + 205, objY, 'Reload JSON', function()
		{
		...
		bpmStepper = new BrainyUINumericStepper(objX, objY, 1, 1, 1, 400, 3);
		...
		scrollSpeedStepper = new BrainyUINumericStepper(objX + 90, objY, 0.1, 1, 0.1, 10, 2);
		scrollSpeedStepper.onValueChange = function() PlayState.SONG.speed = scrollSpeedStepper.value;

		audioOffsetStepper = new BrainyUINumericStepper(objX + 180, objY, 1, 0, -500, 500, 0);
		...
		objY += 40;
		playerDropDown = new BrainyUIDropDownMenu(objX, objY, [''], function(id:Int, character:String)
		{
		...
		stageDropDown = new BrainyUIDropDownMenu(objX + 140, objY, [''], function(id:Int, stage:String)
		{
		...
		opponentDropDown = new BrainyUIDropDownMenu(objX, objY + 40, [''], function(id:Int, character:String)
		{
		...
		girlfriendDropDown = new BrainyUIDropDownMenu(objX, objY + 80, [''], function(id:Int, character:String)
		{
	...
	function addFileTab()
	{
		...
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  New', function()
		{
			var func:Void->Void = function()
			{
				openNewChart();
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Open Chart...', function()
		{
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Open Autosave...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			var radioGrp:BrainyUIRadioGroup = new BrainyUIRadioGroup(0, 0, fileList, 25, maxItems, false, 240);
			radioGrp.checked = 0;
			...
			openSubState(new BasePrompt(420, hei, 'Choose an Autosave',
				function(state:BasePrompt) {
					...
					var btn:BrainyUIButton = new BrainyUIButton(state.bg.x + state.bg.width - 40, state.bg.y, 'X', state.close, 40);
					...
					var btn:BrainyUIButton = new BrainyUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', function()
					{
		...
		{
			btnY += 20;
			var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Open Events...', function()
			{
				if(!fileDialog.completed) return;
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;
				...
				fileDialog.open(function()
				{
					try
					{
						...
						openSubState(new BasePrompt('Events Found! Choose an action.',
							...
							{
								var btnY = 390;
								var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Replace All', function()
								{
									for (event in events)
									{
										if(event != null)
								...
								var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Add', function()
								{
									for (event in loadedEvents)
										events.push(createEvent(event));
								...
								var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Cancel', state.close);
								btn.screenCenter(X);
								btn.x += 125;
								btn.cameras = state.cameras;
								state.add(btn);
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Save', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Save as...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		{
			btnY += 20;
			var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Save Events...', function()
			{
				if(!fileDialog.completed) return;
				upperBox.isMinimized = true;
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Reload Chart', function()
		{
			var func:Void->Void = function()
			{
				if(Song.chartPath == null)
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Save (V-Slice)...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
				...
				try
				{
					...
					openSubState(new BasePrompt('Metadata',
						function(state:BasePrompt)
						{
							...
							var btn:BrainyUIButton = new BrainyUIButton(btnX + 100, btnY, 'Cancel', state.close);
							btn.cameras = state.cameras;
							state.add(btn);
							
							var textX = FlxG.width/2 - 155;
							var textY = 360;
							var artistInput:BrainyUIInputText = new BrainyUIInputText(textX, textY, 120, pack.metadata.artist, 8);
							artistInput.cameras = state.cameras;
							artistInput.onChange = function(old:String, cur:String) pack.metadata.artist = cur;

							var charterInput:BrainyUIInputText = new BrainyUIInputText(textX + 190, textY, 120, pack.metadata.charter, 8);
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Psych to V-Slice...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			fileDialog.open('song.json', 'Open a Psych Engine Chart JSON', function()
			{
				...
				openSubState(new BasePrompt('Metadata',
					function(state:BasePrompt)
					{
						...
						var artistInput, charterInput, difficultiesInput:BrainyUIInputText = null;
						...
						var textY = 360;
						artistInput = new BrainyUIInputText(textX, textY, 120, pack.metadata.artist, 8);
						artistInput.cameras = state.cameras;
						artistInput.onChange = function(old:String, cur:String) pack.metadata.artist = cur;
	
						charterInput = new BrainyUIInputText(textX + 150, textY, 120, pack.metadata.charter, 8);
						charterInput.cameras = state.cameras;
						charterInput.onChange = function(old:String, cur:String) pack.metadata.charter = cur;

						var diffs:Array<String> = pack.metadata.playData.difficulties;
						if(diffs == null || diffs.length < 0) pack.metadata.playData.difficulties = diffs = ['easy', 'normal', 'hard'];
						difficultiesInput = new BrainyUIInputText(textX, textY + 42, 160, diffs.join(', '), 8);
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  V-Slice to Psych...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Update (Legacy)...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Preview (F12)', openEditorPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
		
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Playtest (Enter)', goToPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Exit', function()
		{
	...
	function addEditTab()
	{
		...
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Undo', undo, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Redo', redo, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Select All', function()
		{
			var sel = selectedNotes;
			selectedNotes = curRenderedNotes.members.copy();
			addUndoAction(SELECT_NOTE, {old: sel, current: selectedNotes.copy()});
		...
		if(SHOW_EVENT_COLUMN)
		{
			btnY++;
			btnY += 20;
			var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Lock Events', btnWid);
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Autosave Settings...', btnWid);
		btn.onClick = function()
		{
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new BasePrompt(400, 160, 'Autosave Settings',
				function(state:BasePrompt)
				{
					...
					var checkbox:BrainyUICheckBox = null;
					var timeStepper:BrainyUINumericStepper = null;

					timeStepper = new BrainyUINumericStepper(state.bg.x + 50, state.bg.y + 90, 1, autoSaveCap, 1, 30, 0);
					...
					checkbox = new BrainyUICheckBox(timeStepper.x + 80, timeStepper.y, 'Enabled', 60, function() {
						autoSaveTime = 0;
					...
					var maxFileStepper:BrainyUINumericStepper = new BrainyUINumericStepper(checkbox.x + 140, checkbox.y, 1, backupLimit, 0, 50, 0);
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Clear All Notes', function()
		{
			var func:Void->Void = function()
			{
				resetSelectedNotes();
		...
		{
			btnY += 20;
			var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Clear All Events', function()
			{
				var func:Void->Void = function()
				{
					resetSelectedNotes();
	...
	var showLastGridButton:BrainyUIButton;
	var showNextGridButton:BrainyUIButton;
	var noteTypeLabelsButton:BrainyUIButton;
	var vortexEditorButton:BrainyUIButton;
	function addViewTab()
	{
		...
		showLastGridButton = new BrainyUIButton(btnX, btnY, '', function()
		{
		...
		btnY += 20;
		showNextGridButton = new BrainyUIButton(btnX, btnY, '', function()
		{
		...
		btnY++;
		btnY += 20;
		noteTypeLabelsButton = new BrainyUIButton(btnX, btnY, '', function()
		{
		...
		btnY++;
		btnY += 20;
		vortexEditorButton = new BrainyUIButton(btnX, btnY, vortexEnabled ? '  Vortex Editor ON' : '  Vortex Editor OFF', function()
		{
		...
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Waveform...', function()
		{
			...
			openSubState(new BasePrompt(320, 200, 'Waveform Settings',
				function(state:BasePrompt) {
					...
					var check:BrainyUICheckBox = new BrainyUICheckBox(state.bg.x + 40, state.bg.y + 80, 'Enabled', 60);
					...
					var input:BrainyUIInputText = new BrainyUIInputText(check.x, check.y + 50, 60, waveformC, 10);
					...
					var radioGrp:BrainyUIRadioGroup = new BrainyUIRadioGroup(check.x + 120, check.y, ['Instrumental', 'Main Vocals', 'Opponent Vocals']);
					radioGrp.cameras = state.cameras;
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Go to...', function()
		{
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
				function(state:BasePrompt)
				{
					...
					var timeStepper:BrainyUINumericStepper = new BrainyUINumericStepper(state.bg.x + 100, state.bg.y + 90, 1, Math.floor(curTime)/1000, 0, FlxG.sound.music.length/1000 - 0.01, 2, 80);
					timeStepper.cameras = state.cameras;
					var sectionStepper:BrainyUINumericStepper = new BrainyUINumericStepper(timeStepper.x + 160, timeStepper.y, 1, currentSec, 0, PlayState.SONG.notes.length - 1, 0);
					...
					var btn:BrainyUIButton = new BrainyUIButton(0, timeTxt.y + 30, 'Go To', function()
					{
						curSec = currentSec;
					...
					var btn:BrainyUIButton = new BrainyUIButton(0, btn.y, 'Cancel', state.close);
					btn.cameras = state.cameras;
					btn.screenCenter(X);
					btn.x += 60;
					state.add(btn);
		...
		btnY++;
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Theme...', function()
		{
			if(!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			openSubState(new BasePrompt(500, 260, 'Chart Editor Theme',
				function(state:BasePrompt)
				{
					...
					var btnY = 320;
					var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Light', changeTheme.bind(LIGHT));
					btn.screenCenter(X);
					btn.x -= 180;
					btn.cameras = state.cameras;
					state.add(btn);
			
					var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Dark', changeTheme.bind(DARK));
					btn.screenCenter(X);
					btn.x -= 60;
					btn.cameras = state.cameras;
					state.add(btn);
					
					var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Default', changeTheme.bind(DEFAULT));
					btn.screenCenter(X);
					btn.cameras = state.cameras;
					btn.x += 60;
					state.add(btn);
			
					var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'V-Slice', changeTheme.bind(VSLICE));
					btn.screenCenter(X);
					btn.x += 180;
					btn.cameras = state.cameras;
					state.add(btn);
					...
					btnY += 60;
					var btn:BrainyUIButton = new BrainyUIButton(0, btnY, 'Custom', changeTheme.bind(CUSTOM));
					btn.screenCenter(X);
					btn.x -= 180;
					btn.cameras = state.cameras;
					state.add(btn);
					...
					var input:BrainyUIInputText = new BrainyUIInputText(0, btnY, 80, customBgC, 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					...
					var input:BrainyUIInputText = new BrainyUIInputText(0, btnY, 80, customGridC[0], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
					...
					var input:BrainyUIInputText = new BrainyUIInputText(0, btnY + 30, 80, customGridC[1], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
					...
					var input:BrainyUIInputText = new BrainyUIInputText(0, btnY, 80, customGridOtherC[0], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
					...
					var input:BrainyUIInputText = new BrainyUIInputText(0, btnY + 30, 80, customGridOtherC[1], 10);
					input.maxLength = 6;
					input.filterMode = ONLY_HEXADECIMAL;
					input.forceCase = UPPER_CASE;
					input.screenCenter(X);
		...
		btnY += 20;
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  Reset UI Boxes', function()
		{
			mainBox.setPosition(mainBoxPosition.x, mainBoxPosition.y);
			infoBox.setPosition(infoBoxPosition.x, infoBoxPosition.y);
			UIEvent(BrainyUIBox.DROP_EVENT, btn); //to force a save
		}, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
	}
	...
	override public function UIEvent(id:String, sender:Dynamic)
	{
		...
		switch(id)
		{
			case BrainyUIButton.CLICK_EVENT, BrainyUIDropDownMenu.CLICK_EVENT:
				ignoreClickForThisFrame = true;

			case BrainyUIBox.CLICK_EVENT:
				ignoreClickForThisFrame = true;
				if(sender == upperBox) updateUpperBoxBg();

			case BrainyUIBox.MINIMIZE_EVENT:
				if(sender == upperBox)
				{
